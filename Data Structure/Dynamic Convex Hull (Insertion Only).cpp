...
inline int sign(LL x) { return x < 0 ? -1 : int(x > 0); }
struct point {
	int x, y;
	point(): x(0), y(0) {}
	...
	friend inline LL det(const point &a, const point &b) { return (LL)a.x * b.y - (LL)a.y * b.x; }
};
...
int main() {
	...
	for (int i = 1; i <= N; ++i) {
		...
		if (t == 1) {
			...
			if (checkInside(upperHull, point(x, y)) && checkInside(lowerHull, point(x, -y))) puts("YES");
			else puts("NO");
		}
	}
	return 0;
