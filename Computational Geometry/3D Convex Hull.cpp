namespace ConvexHull3D {
	...
	vector<Facet> getHull(int n, point ps[]) {
		static int mark[MAXN][MAXN], a, b, c; int stamp = 0; bool exist = false;
		vector<Facet> facet; random_shuffle(ps, ps + n);
		for (int i = 2; i < n && !exist; i++) {
			point ndir = det(ps[0] - ps[i], ps[1] - ps[i]);
			if (ndir.len() < EPS) continue;
			swap(ps[i], ps[2]); for (int j = i + 1; j < n && !exist; j++)
				if (sign(volume(0, 1, 2, j)) != 0) {
					exist = true; swap(ps[j], ps[3]);
					facet.push_back(Facet(0, 1, 2)); facet.push_back(Facet(0, 2, 1));
				}
		} if (!exist) return ConvexHull2D(n, ps);
		for (int i = 0; i < n; ++i) for (int j = 0; j < n; ++j) mark[i][j] = 0;
		stamp = 0; for (int v = 3; v < n; ++v) {
			vector<Facet> tmp; ++stamp;
			for (unsigned i = 0; i < facet.size(); i++) {
				a = facet[i].a; b = facet[i].b; c = facet[i].c;
				if (sign(volume(v, a, b, c)) < 0)
					mark[a][b] = mark[a][c] = mark[b][a] = mark[b][c] =  mark[c][a] = mark[c][b] = stamp;
...
namespace Gravity {
	...
	point findG(point ps[], const vector<Facet> &facet) {
		...
		for (int i = 0, size = facet.size(); i < size; ++i) {
			...
			point p = (a + b + c + o) * 0.25; double w = mix(a - o, b - o, c - o);
			ws += w; res = res + p * w;
		} res = res / ws;
		return res;
	}
