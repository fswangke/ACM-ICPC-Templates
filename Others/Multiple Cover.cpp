namespace DLX {
	...
	inline bool cmpByX(const link &a, const link &b) { return a->x < b->x; }
	inline bool cmpByY(const link &a, const link &b) { return a->y < b->y; }
	...
	inline void addElement(int x, int y) {
		...
		eachRow[x].push_back(top);
		eachCol[y].push_back(top++);
	}

	void init(int _row, int _col, int _nGE) {
		row = _row; col = _col; nGE = _nGE;
		top = base; stamp = 0;
