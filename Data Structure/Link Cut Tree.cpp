...
inline void Cut(tree t) {
	Access(t); Splay(t);
	t->ch[0]->pre = null; t->ch[0] = null;
	Update(t);
}
inline void Cut_simp(tree a, tree b) { // `A simpler version`
	MakeRoot(a); Access(a); Splay(b); b->pre = null;
}
inline void Cut(tree x, tree y) {
	tree upper = (Access(x), Access(y));
	if (upper == x) {
		Splay(x);
		y->pre = null; x->ch[1] = null;
		Update(x);
	} else if (upper == y) {
		Access(x); Splay(y);
