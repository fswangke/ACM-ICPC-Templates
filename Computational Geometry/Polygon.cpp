struct Polygon { // stored in [0, n)
	...
	bool convexContain(const point &p) const { // sort by polar angle
		...
		while (l <= r) {
			int mid = (l + r) >> 1;
			double d1 = sign(det(list[mid], q)), d2 = sign(det(list[mid - 1], q));
			if (d1 <= 0) {
				if (d2 <= 0) {
	...
	bool isPL(const point &a, const point &b, point &cp1, point &cp2) {  // `$O (log N)$`
		...
		double k1 = det(b - a, list[i] - a), k2 = det(b - a, list[j] - a);
		if (sign(k1) * sign(k2) > 0) return false; // `no intersection`
		if (sign(k1) == 0 || sign(k2) == 0) { // `intersect with a point or a line in the convex`
			if (sign(k1) == 0) {
				if (sign(det(b - a, list[i + 1] - a)) == 0) cp1 = list[i], cp2 = list[i + 1];
				else cp1 = cp2 = list[i];
				return true;
			}
			if (sign(k2) == 0) {
				if (sign(det(b - a, list[j + 1] - a)) == 0) cp1 = list[j], cp2 = list[j + 1];
				else cp1 = cp2 = list[j];
			}
			return true;
		}
