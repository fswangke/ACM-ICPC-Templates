namespace Blossom {
	...
	void init(int _n) { n = _n; for (int i = 0; i < n; ++i) link[i].clear(); }
	...
	int findCommonAncestor(int x, int y) {
		...
		for ( ; ; x = pred[ match[x] ]) {
			x = label[x]; inPath[x] = true; if (x == S) break;
		}
		for ( ; ; y = pred[ match[y] ]) {
			y = label[y]; if (inPath[y]) break;
		} return y;
	}
	void resetTrace(int x, int lca) {
		while (label[x] != lca) {
			int y = match[x]; inb[ label[x] ] = inb[ label[y] ] = true;
	...
	void blossomContract(int x, int y) {
		...
		resetTrace(x, lca); resetTrace(y, lca);
		if (label[x] != lca) pred[x] = y;
		if (label[y] != lca) pred[y] = x;
		for (int i = 0; i < n; ++i) if (inb[ label[i] ]) {
			label[i] = lca; if (!inq[i]) push(i);
		}
	}
	...
	int findMaxMatching() {
		...
		for (S = 0; S < n; ++S) if (match[S] == -1)
			if (findAugmentingPath()) ++ans;
		return ans;
	}
}